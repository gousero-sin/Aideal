...
const fields = [
  {
    key: 'saldo_ano_anterior',
    label: 'Saldo do Ano Anterior',
    hint: 'Valor anual usado na geração do Fluxo de Caixa',
  },
];
...
export default function FluxoIndicatorsAdminPanel({ apiBase, onNotify, onBusyChange }) {
  ...
  const carregarIndicadores = async (anoRef = ano, options = {}) => {
    ...
    setError(null);
    try {
      const response = await fetch(`${apiBase}/fluxo_caixa/admin/indicadores?ano=${anoRef}`, {
        credentials: 'same-origin',
      });
      if (!response.ok) {
        const detail = await response.json().catch(() => ({}));
        throw new Error(detail?.detail || 'Erro ao consultar indicadores do Fluxo.');
      }
      const payload = await response.json();
      if (requestId !== requestIdRef.current) return;
      const indicadores = payload?.indicadores || {};
      setForm({
        saldo_ano_anterior: inputValue(indicadores.saldo_ano_anterior),
      });
      setRecordInfo({
  ...
  useEffect(() => {
    carregarIndicadores(ano, { mostrarLoading: false });
  ...
  const handleSubmit = async (event) => {
    ...
    try {
      ...
      const response = await fetch(`${apiBase}/fluxo_caixa/admin/indicadores`, {
        method: 'POST',
        credentials: 'same-origin',
        body: formData,
      });
      if (!response.ok) {
        const detail = await response.json().catch(() => ({}));
        throw new Error(detail?.detail || 'Erro ao salvar indicadores do Fluxo.');
      }
      const payload = await response.json();
      const indicadores = payload?.indicadores || {};
      setForm({
      ...
      onNotify?.({ type: 'success', message: `Indicadores Fluxo ${ano} salvos.` });
    } catch (err) {
      setError(err.message);
      onNotify?.({ type: 'error', message: err.message });
    } finally {
    ...
    <div className="aideal-admin-grid">
      <aside className="aideal-admin-sidebar">
        ...
        <div className="aideal-bank-status">
          ...
          <p>
            {recordInfo.existe
              ? `Registro disponível para ${ano}.`
              : `Sem registro salvo para ${ano}.`}
          </p>
        </div>
      </aside>
      ...
      <form className="aideal-admin-main aideal-indicators-form" onSubmit={handleSubmit}>
        <header className="aideal-workspace-head">
          <div>
            <h3>Fluxo de Caixa · indicadores manuais</h3>
            <p>Saldo anual usado como base do documento de Fluxo de Caixa.</p>
          </div>
        ...
        <span className={`aideal-competencia-status is-${recordInfo.existe ? 'success' : 'manual'}`}>
          {loading
            ...
            : recordInfo.updatedAt
              ...
              : 'Informe o valor anual e confirme o salvamento.'}
        </span>
        ...
        <div className="aideal-indicator-summary">
          <span>
            ...
            {recordInfo.existe ? 'Consulta carregada do banco' : 'Novo registro anual'}
          </span>
          <span>
            <WalletCards size={15} aria-hidden="true" />
            Base manual do Fluxo de Caixa
          </span>
        </div>
